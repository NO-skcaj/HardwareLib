    ...
    class TalonFX : public Motor
    {
        public:
            ...
            inline void ConfigureMotor(MotorConfiguration& config) override // Configure the motor with default settings
            {
                ...
                slot0Configs.kP = config.PID.P;
                slot0Configs.kI = config.PID.I;
                slot0Configs.kD = config.PID.D;
                slot0Configs.kS = config.FeedForward.has_value() ? config.FeedForward->S : 0.0;
                slot0Configs.kV = config.FeedForward.has_value() ? config.FeedForward->V : 0.0;
                slot0Configs.kA = config.FeedForward.has_value() ? config.FeedForward->A : 0.0;
                
                // Configure Motion Magic
                ctre::phoenix6::configs::MotionMagicConfigs &motionMagicConfigs = talonFXConfiguration.MotionMagic;
                motionMagicConfigs.MotionMagicCruiseVelocity = config.MagicMotion.has_value() ? config.MagicMotion.value().TargetVelocity : 0.0_tps;
                motionMagicConfigs.MotionMagicAcceleration   = config.MagicMotion.has_value() ? config.MagicMotion.value().TargetAcceleration : 0.0_tr_per_s_sq; 
                motionMagicConfigs.MotionMagicJerk           = config.MagicMotion.has_value() ? config.MagicMotion.value().TargetJerk : 0.0_tr_per_s_cu; 
